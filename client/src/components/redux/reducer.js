...
const rootReducer = (state=initialState, action)=>{//Action es la inforamcion llegara desde la seccion de actions con el contenido del payload;
    ...
    case SORT_BY_NAME:
        ...
        return state.filter_by_temps?{...state, filter_by_temps: sort}:{...state, breed:sort}
    case SORT_BY_WEIGHT:
             let sorted = ''
        ...
        if(action.payload === 'Max-Min'){ //Al despertar identificar los NaN del array;
            ...
            return state.filter_by_temps?{...state, filter_by_temps:sorted}:{...state,breed:sorted}
    case GET_TEMPERAMENTS:
        return {...state, temperaments: action.payload}
    case GET_FILTER_BY_TEMPS:
       let filtrado = '';
       if(action.payload === 'All'){
        return{...state, filter_by_temps:''}
       }
       if(action.payload !== 'All'){
        filtrado = state.breed.filter(a=> a.temperaments.includes(action.payload));
       }
       if(filtrado.length){
